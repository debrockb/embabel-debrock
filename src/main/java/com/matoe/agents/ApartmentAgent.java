...
/**
 * Apartment / short-term rental search agent — searches Airbnb, Vrbo, Booking.com
        ...
        if (browserService.isAvailable()) {
            try {
                ...
                if (raw != null && !raw.isEmpty()) {
                    log.info("ApartmentAgent: {} results via browser for {}", raw.size(), request.destination());
                    return raw.stream().map(m -> map(m, nights, "browser")).collect(Collectors.toList());
                }
            } catch (Exception e) {
                log.warn("ApartmentAgent browser search failed, falling back to LLM: {}", e.getMessage());
            }
        }

        // ── fallback: LLM-generated results (marked as source=llm) ───────────
        try {
            String systemPrompt = dynamicPromptService.getPrompt("apartment-agent");
            if (systemPrompt.isBlank()) systemPrompt = defaultPrompt;
            String userPrompt = promptTemplateService.buildApartmentPrompt(systemPrompt, request);

            long start = System.currentTimeMillis();
            String raw = llmService.call(model, "You are a travel search expert. Return ONLY valid JSON array.", userPrompt);
            long durationMs = System.currentTimeMillis() - start;

            costTracker.logCall(request.sessionId(), "apartment-agent", model != null ? model : "default",
            ...
            log.warn("ApartmentAgent LLM fallback failed for {}: {}", request.destination(), e.getMessage());
            costTracker.logCall(request.sessionId(), "apartment-agent", request.extractorModel(),
